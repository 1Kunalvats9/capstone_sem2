    ...
    <nav className='w-full flex mt-6 items-center justify-between'>
      ...
      <ul className='text-white hidden md:flex list-none items-center gap-5'>
        ...
        <li className='cursor-pointer hover:text-[#38BDF9] duration-150' onClick={() => { router.push(isLoggedIn ? "/properties" : "/login") }}>Properties</li>
        {
          isLoggedIn &&
          <>
            <li className='cursor-pointer hover:text-[#38BDF9] duration-150' onClick={() => { router.push("/profile") }}>My Profile</li>
            <li className='cursor-pointer hover:text-[#38BDF9] duration-150' onClick={() => { router.push("/add-property") }}>Add Property</li>
          </>
        }
      </ul>
      <div className='md:flex items-center hidden gap-4'>
        {
          isLoggedIn ?
            ...
            <>
              ...
              <button onClick={handleLogin} className='bg-[#0084C7] text-white px-4 py-1 text-md rounded-lg cursor-pointer hover:opacity-85 duration-200'>Register</button>
            </>
        }
      </div>
