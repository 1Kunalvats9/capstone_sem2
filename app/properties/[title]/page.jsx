...
import {MapPin, Bed, Bath, Maximize, Clock, DollarSign, User, Calendar, Home, IndianRupee} from 'lucide-react';
...
const PropertyDetailPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchPropertyDetails = async () => {
      // Ensure title and email are present before fetching
      if (title && email) {
        setLoading(true);
        try {
  ...
  }, [params.title, email]); // Dependency array optimized to prevent re-fetching
  ...
  const handlePlaceBid = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setBidAmount('');
        // The new bid will be received via socket, so no need to fetch here
      } else {
          ...
          <div className="max-w-6xl mx-auto py-10">
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              <div className="lg:col-span-2">
                <div className="relative w-full h-96 rounded-lg overflow-hidden mb-6 glass-effect">
                  ...
                  {onlineUsers > 0 && (
                      <div className="absolute top-4 right-4 glass-effect px-3 py-1 rounded-full flex items-center gap-2">
                      ...
                      <div className="mt-6">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {property.amenities.map((amenity, index) => (
                              <span
                                  key={index}
                                  className="px-3 py-1 glass-effect-deep rounded-full text-sm text-slate-300"
                              >
                          {amenity}
                        </span>
                          ))}
                      ...
                      {timeLeft !== 'Bidding Ended' && (
                          <div className="space-y-3">
                            <input
                                ...
                                className="w-full px-4 py-3 glass-effect-deep border-slate-600 rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                            />
                            <button
                  ...
                  {bids.length > 0 ? (
                      <div className="space-y-3 max-h-64 overflow-y-auto">
                        {bids.slice(0, 10).map((bid, index) => (
                            <div key={bid._id || index} className="flex justify-between items-center p-3 glass-effect-deep rounded-lg border border-slate-700">
                              <div>
